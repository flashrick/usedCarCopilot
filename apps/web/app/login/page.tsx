      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
        <section className="surface-card p-5 md:p-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              <h1 className="font-[var(--font-space-grotesk)] text-3xl font-semibold text-textStrong">{copy.auth.loginTitle}</h1>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-textBody">{copy.auth.loginDescription}</p>
            </div>
          ...
          <form onSubmit={handleSubmit} className="mt-6 grid gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button type="submit" disabled={submitting} className="btn-primary h-11">
                {submitting ? copy.auth.loading : copy.auth.submitLogin}
                <ArrowRight className="h-4 w-4" />
              </button>
              <Link href={`/register?next=${encodeURIComponent(nextPath)}`} className="btn-secondary h-11">
