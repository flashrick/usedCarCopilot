...
from app.evaluation.recommendation_eval import (
    RecommendationEvalConfig,
...
def main() -> None:
    ...
    parser.add_argument(
        "--fail-under-citation-score",
        type=float,
        default=None,
        help="Exit with failure when average citation score is below this threshold.",
    )
    parser.add_argument(
    ...
    failures: list[str] = []
    if args.fail_under_citation_score is not None and summary["average_citation_score"] < args.fail_under_citation_score:
        failures.append(
            f"average citation score {summary['average_citation_score']:.2%} "
            f"is below {args.fail_under_citation_score:.2%}"
        )
    ...
    if failures:
        print("Recommendation eval failed thresholds")
